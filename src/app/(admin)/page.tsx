...
function DashboardContent() {
    ...
    const iPhoneUsed = iPhones.filter(i => i.status === 'in-use').length;

    // 2. Feature Phone Logic
    const featurePhoneTotal = featurePhones.length;
    const featurePhoneUsed = featurePhones.filter(f => f.status === 'in-use').length;

    // 3. Router Logic
    const routerTotal = routers.length;
    const routerUsed = routers.filter(r => r.status === 'in-use').length;
